    ...
    internal static func == (lhs: _RangeSetStorage, rhs: _RangeSetStorage) -> Bool
    {
        switch (lhs._storage, rhs._storage)
        {
            ...
            case (.empty, .singleRange), (.singleRange, .empty):
                return false
            case let (.empty, .variadic(ranges)), let (.variadic(ranges), .empty):
                return ranges.isEmpty      
            case let (.singleRange(lhs), .singleRange(rhs)):
                return lhs == rhs
            case let (.singleRange(singleRange), .variadic(ranges)), let (.variadic(ranges), .singleRange(singleRange)):
                return ranges.count == 1 && (ranges[0]) == singleRange      
...
{
    internal mutating func replaceSubrange<C>(_ subrange: Range<Int>, with newElements: C)
