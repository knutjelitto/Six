...
fileprivate struct _CapacityAndFlags
{
    ...
    internal init(hasBreadcrumbs crumbs: Bool, realCapacity: Int)
    {
        ...
        _internalInvariant(crumbs == self.hasBreadcrumbs && realCapacity == self._realCapacity)
    }
    ...
    fileprivate var hasBreadcrumbs: Bool { (self._storage & _CapacityAndFlags.hasBreadcrumbsMask) != 0 }
}
