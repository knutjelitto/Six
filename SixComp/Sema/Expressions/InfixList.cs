...
{
    public class InfixList : Expression<Tree.Expression>
    {
        public InfixList(IScoped outer, Tree.Expression tree)
            : base(outer, tree)
        {
        ...
        public void MakeInfix()
        {
            ...
            IExpression MakeOperation(IOperator op, IExpression left, IExpression right)
            {
                if (op is IOperator.Conditional cond)
                {
                    ...
                    return new Conditional(Outer, op, left, mid, right);
                }
                return new Infix(Outer, op, left, right);
            }
        }
